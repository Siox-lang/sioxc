...
use crate::ir::{BinOp, Design, SignalId};
...
impl Testbench<'_> {
    ...
    fn alias_ids(&self, name: &str) -> Vec<crate::ir::SignalId> {
    ...
    fn eval_free_call(
        ...
        let consts: Option<Vec<i64>> = args
            .iter()
            .map(|a| crate::ir::eval_const_fns(a, &HashMap::new(), self.fns, 0))
            .collect();
        if let Some(cs) = consts {
            ...
            if let Some(v) = crate::ir::eval_const_stmts(&body.stmts, &cenv, self.fns, 0) {
                return v as u128;
            }
        }
