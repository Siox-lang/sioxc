...
`diag` → `syntax` → `resolve` → `types` → `elab` → `ir`

`testbench` and `wave` are small consumers of IR/type metadata rather than
compiler stages; the native test harness is generated by `sioxc`.

Inside the package this is enforced by module discipline. No upward or sideways
`use`.
