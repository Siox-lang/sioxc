...
impl<'a> Elaborator<'a> {
    ...
    fn resolve_connections(
        ...
        // A missing input connection is NOT a compile-time error: whether a
        // port ends up driven (by the block, a post-declaration `inst.p = x;`,
        // or conditionally) is a property of the running design, not something
        // to prove statically. An input left undriven simply reads its default
        // at runtime — any resulting error surfaces there, not here.
        conns
    }
...
mod tests {
    ...
    #[test]
    fn missing_connection_is_not_an_error() {
        // `rst` is left unconnected. Connectedness is a runtime property (the
        // port may be driven post-declaration or conditionally, or read as its
        // default), so elaboration does not reject it.
        let src = "module m;\n\
