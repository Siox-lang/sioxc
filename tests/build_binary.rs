...
fn decode_fst(fst: &std::path::Path) -> String {
    ...
    let build = Command::new("clang")
        ...
        .arg(root.join("third_party/libfst/src/fstapi.c"))
        .arg(root.join("third_party/libfst/src/fastlz.c"))
        .arg(root.join("third_party/libfst/src/lz4.c"))
        .arg("-I")
        .arg(root.join("third_party/libfst/src"))
        .args(["-O2", "-lz", "-o"])
        .arg(&decoder)
        .output()
        .unwrap();
